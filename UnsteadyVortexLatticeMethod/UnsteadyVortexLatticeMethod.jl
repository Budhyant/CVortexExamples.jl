...
let
	println("Running UnsteadyVortexLatticeMethod.jl")
	println("A Demo of the UVLM using CVortex.")
	
    aoa = deg2rad(0)
	println("AoA is ", aoa, " radians")
    ...
    dt = 0.1
    steps = 500
	println("Computing ", steps, " steps.")
	println("Wing geometry is discretised into ", 
		(length(wing_xdisc)-1)*(length(wing_ydisc)-1), " vortex rings")
    ...
    function save_mesh(i, lattices::Vector{VortexLattice})
        ...
        vtkfile = WriteVTK.vtk_grid("VLM_rings_"*string(i), 
            points', cells)
        WriteVTK.vtk_cell_data(vtkfile, cell_strs, "Ring_strengths")
        WriteVTK.vtk_save(vtkfile)
    end
	println("Output will be written to VLM_rings_[step].vtk")
