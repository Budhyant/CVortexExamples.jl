##############################################################################
#
# Particle2DCollisionsWithRedistVisc.jl
#
# Models sets of particles colliding in 2D with redistribution and viscosity.
# Outputs to particles2D_WRV_STEP.vtu.
#
...
let
    ...
    period = 1
    offset = 0.5
	kinematic_visc = 1. / 1000.

	dx = 0.0025
    xs = collect(1: dx :5)
    ys = collect(-0.000 : dx : 0.0025)
    
    particle_pos = zeros(length(xs) * length(ys)*4, 2)
    particle_vorts = ones(size(particle_pos)[1]) * vorticity
	particle_areas = ones(size(particle_pos)[1]) * dx^2
    acc = 1
    for j = 1:length(ys)
        for i = 1:length(xs)
    ...
    function save_particles(step)
        ...
        cells = map(
            ...
            1:size(pos3d)[1])
        vtkfile = WriteVTK.vtk_grid("particles2D_WRV_"*string(step), 
            pos3d', cells)
    ...
    for i = 1 : n_steps
		...
		dvort = particle_visc_induced_dvort(particle_pos, particle_vorts,
			particle_areas, particle_pos, particle_vorts, particle_areas,
			kernel, regdist, kinematic_visc)
        tend = now()
        print("\rStep:\t", i, "\tInteractions per second: ", 
            1000*round(ninter / Float64((tend - tstart).value)),"\t\t\t")
        particle_pos .+= dt .* vels
		particle_vorts .+= dt .* dvort
		if i % 10 == 0
			particle_pos, particle_vorts, particle_areas = redistribute_particles_on_grid(
